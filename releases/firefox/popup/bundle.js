...
const favIconFallback = exports.favIconFallback = '/assets/file.svg';
const alertCircle = exports.alertCircle = '/assets/alert-circle.svg';
const deleteButton = exports.deleteButton = document.querySelector('.delete-circle');
const searchInput = exports.searchInput = document.querySelector('.search');
const tabList = exports.tabList = document.querySelector('.tab-list');
...
// Initialize tabs
(0, _domUtils.populateTabList)();

// Set a timeout on focus so input is focused on popup
document.addEventListener('DOMContentLoaded', () => setTimeout(() => _constants.searchInput.focus(), 150));
// Any (non-navigating) keydown should activate the search field,
window.addEventListener('keydown', handleKeyDown);
_constants.searchInput.addEventListener('change', updateSearchResults);
_constants.searchInput.addEventListener('keyup', updateSearchResults);
_constants.deleteButton.addEventListener('click', event => {
  (0, _domUtils.clearInput)(event.target);
  _constants.tabList.childNodes[0].focus();
});
...
function handleKeyDown(event) {
  switch (event.key) {
    ...
    case 'ArrowRight':
    case 'ArrowLeft':
      (0, _domUtils.navigateResults)(event.key);
      break;
    case 'Enter':
      event.preventDefault();
      if (!document.activeElement.className.includes('tab-object') && !document.activeElement.className.includes('no-result')) {
        _constants.tabList.childNodes[0].focus();
        setTimeout(() => (0, _domUtils.switchActiveTab)(document.activeElement.dataset.id), 150);
      } else {
        (0, _domUtils.switchActiveTab)(document.activeElement.dataset.id);
      }
      break;
    case 'Escape':
      // This only works in chrome, in firefox it always closes the window
      if (_constants.searchInput.value.length === 0) {
        window.close();
      } else {
        (0, _domUtils.clearInput)();
      }
      break;
    default:
      _constants.searchInput.focus();
      break;
  }
}
...
function updateSearchResults(event) {
  ...
  if (_constants.searchInput.value.length === 0) {
    _constants.deleteButton.classList.add('hidden');
  } else {
    _constants.deleteButton.classList.remove('hidden');
  }

  const query = event.target.value.trim().toLowerCase();
  const getSearchResults = (0, _search2.default)(query);
  return (0, _domUtils.getAllTabs)().then(getSearchResults).then(_domUtils.injectTabsInList);
}
...
exports.getAllTabs = undefined;
exports.switchActiveTab = switchActiveTab;
exports.clearInput = clearInput;
exports.populateTabList = populateTabList;
exports.tabToTag = tabToTag;
exports.getNodePositions = getNodePositions;
exports.createNoResult = createNoResult;
exports.injectTabsInList = injectTabsInList;
exports.navigateResults = navigateResults;
...
const getAllTabs = exports.getAllTabs = initializeTabs();
...
function tabToTag({ favIconUrl, title, id, url }) {
  const isValidFavIconUrl = favIconUrl && !isChromeLink(favIconUrl) && !badFavIcons().includes(favIconUrl);
  const favIconLink = isValidFavIconUrl ? favIconUrl : _constants.favIconFallback;
  // Just check if it's a url for now so we can shorten it
  const tabTitle = isURL(title) ? getBasePath(title) : title;
...
function handleBadSvg() {
  ...
  this.src = _constants.favIconFallback;
}
...
function getNodePositions(parentNode, selectedNode) {
  return {
    ...
    selectedBottom: selectedNode.offsetHeight + (selectedNode.offsetTop - parentNode.offsetTop) - parentNode.scrollTop,
    selectedTop: selectedNode.offsetTop - parentNode.offsetTop - parentNode.scrollTop
  };
}
...
function createNoResult() {
  ...
  alertCircleNode.setAttribute('data', _constants.alertCircle);
...
function injectTabsInList(tabArray) {
  const wasNoResult = _constants.tabList.querySelectorAll('.tab-object').length === 0;
  ...
  clearChildren(_constants.tabList);

  // Add nodes to tabList & attach event listeners
  if (showNoResult) {
    _constants.tabList.appendChild(createNoResult());
  } else {
    tabArray.map(tabToTag).forEach(tabNode => {
      ...
      _constants.tabList.appendChild(tabNode);
    });
  }
}
    ...
    case 'ArrowDown':
      {
        ...
        if (nextSibling) {
          ...
          _constants.tabList.querySelector('.tab-object').focus();
        }
        break;
      }
    case 'ArrowLeft':
...
function filterResults(query) {
  ...
  const fuse = array => new _fuse2.default(array, options);
